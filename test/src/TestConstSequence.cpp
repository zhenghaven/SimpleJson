...
GTEST_TEST(TestConstSequence, UInt64Array)
{
	...
	static_assert(SIMPLEJSON_CONST_ARRAY(testCArray)::sk_size == 10, "ConstSequence Impl Error!");

	auto outData = SIMPLEJSON_CONST_ARRAY(testCArray)::GetData();
	EXPECT_EQ(outData, testArray);
}

GTEST_TEST(TestConstSequence, String)
{
	static_assert(SIMPLEJSON_CONST_STRING("Test_string_1")::sk_size == 13, "ConstSequence Impl Error!");
