...
GTEST_TEST(TestStaticSequence, UInt64Array)
{
	...
	static_assert(SIMPLEJSON_STATIC_ARRAY(testCArray)::sk_size == 10, "StaticSequence Impl Error!");

	auto outData = SIMPLEJSON_STATIC_ARRAY(testCArray)::GetData();
	EXPECT_EQ(outData, testArray);
}

GTEST_TEST(TestStaticSequence, String)
{
	static_assert(SIMPLEJSON_STATIC_STRING("Test_string_1")::sk_size == 13, "StaticSequence Impl Error!");

	auto outStr = SIMPLEJSON_STATIC_STRING("Test_string_1")::GetStr();
